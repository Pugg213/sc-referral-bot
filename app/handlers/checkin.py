...
async def daily_checkin_callback(callback: types.CallbackQuery):
    ...
    try:
        with db.get_connection() as conn:
            ...
            # Начисляем награду пользователю (обновляем ОБА баланса для консистентности)
            cursor.execute("""
                UPDATE users 
                SET total_earnings = total_earnings + ?,
                    ...
                    balance = balance + ?
                WHERE user_id = ?
            """, (checkin_reward, checkin_reward, checkin_reward, user_id))
            ...
            # Получаем новый баланс (используем основной баланс для консистентности)
            cursor.execute("""
